...
const EditUserForm = ({ user }) => {
  ...
  let canSave;
  if (password) {
    // if there is a pw, has to be validPassword
    canSave =
      [roles.length, validUsername, validPassword].every(Boolean) && !isLoading;
  } else {
    // otherwise we only need the below to make canSave true
  ...
  // error content, based on error, or delete mutation, if neither (??) then dont add anything
  // errContent would be empty then if neither
